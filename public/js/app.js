 ...
 * FINAL STABLE VERSION: Full Feature Set Integration with Crash Avoidance Logic
 */
...
function initGoals() {
    ...
    // ★タブ要素のイベント設定 (initGoals実行時に要素が存在するか確認)
    ...
    const ren = () => {
        ...
        const targets = State.activeGoals.filter(g => { 
            // フィルタリングロジックの強化（タブが存在しない場合も考慮）
            const isTabExist = tabActive && tabHistory;
            if (isTabExist && currentTab === 'active') return !g.status; 
            else if (isTabExist && currentTab === 'history') return g.status === '達成' || g.status === '中止';
            else return true; // タブ要素がHTMLにない場合は、全目標を表示する (互換性維持)
        });

        if(targets.length === 0) { 
            lst.innerHTML = `<p class="text-center text-gray-400 mt-10">${(tabActive && currentTab === 'history') ? '履歴はありません 📜' : '進行中の目標はありません 🌱'}</p>`; 
        }
        ...
        targets.forEach(g => {
            try { 
                ...
                const btnContainer = t.querySelector('.button-container'); // ★ボタンコンテナの取得
                ...
                if(btnContainer) {
                    ...
                    if (currentTab === 'active' || !tabActive) { 
                        // 進行中タブのボタン (tabActiveがない場合もこちらでレンダリングする)
