...
async function fetchLLM(prompt, mode = 'analysis') {
    ...
    if (mode === 'analysis') {
        ...
        "challengeAI": 1-7,
        "skillAI": 1-7,
        "reasonAI": "分析結果と根拠（PEOの観点を含めて、改行せず簡潔に）",
...
async function startGoalConsultation() {
    ...
    addMsg("こんにちは！一緒に目標を考えましょう！✨ \n最近やってみたいことや、気になっていることはありますか？", 'bot');
...
function initRecord() {
    ...
    form.onsubmit = async(e) => {
        ...
        // ★修正ポイント: ユーザーの入力を表示し、画面を切り替えてからAIを呼ぶ
        const p = `目標: ${getGoalMainText(State.selectedGoal.goal)}\n自己評価: 挑戦${c}/能力${s}\n理由: ${r}`;
        addChatMessage(p.replace(/\n/g, '<br>'), 'user');
        form.classList.add('hidden'); // 先に隠す
        chatArea.classList.remove('hidden'); // 先にチャットを出す
