...
function initGoals() {
    ...
    if(tabActive && tabHistory) {
        ...
        // タブ要素が存在しない場合（初期のindex.html）は、レンダリングのみ実行
        ren();
    }
    ...
    const ren = () => {
        ...
        const targets = State.activeGoals.filter(g => { 
            // フィルタリングロジックの強化（tabActiveが存在しない場合も考慮）
            if (tabActive && currentTab === 'active') return !g.status; 
            else if (tabActive && currentTab === 'history') return g.status === '達成' || g.status === '中止';
            else return true; // タブが存在しない場合は全目標を表示
        });
